(function(){
  ...
  const labels={
    he:{items:'מוצרים מוצגים', empty:'הסל ריק. הוסיפו פריטים מהקטלוג לפני שליחת בקשת הזמנה.', miniEmpty:'הסל ריק.', sample:'פריט קטלוג סחר', remove:'הסרה', added:'נוסף לסל', add:'הוספה לסל', qty:'כמות', selected:'פריטים שנבחרו', view:'המשך להזמנה', emptyCart:'ריקון סל', unit:'מחיר יחידה', carton:'מחיר ליחידה בקרטון מלא'}
  };
  const lang=()=> 'he';
  const L=()=>labels[lang()];
  const UI=()=>({sku:'מק״ט', brand:'מותג', category:'קטגוריה', unitPerCarton:'יחידות בקרטון', warranty:'אחריות', availability:'זמינות', toConfirm:'לאישור', unit:'מחיר יחידה', carton:'מחיר ליחידה בקרטון מלא', total:'סה״כ', add:'הוספה לסל', brands:'מותגים', loading:'טוען קטלוג Secure Smart', loadingTail:'מרשימות המותגים המאושרות…', loaded:'מוצרים נטענו', loadedTail:'מרשימות המותגים המאושרות של Secure Smart.', noMatch:'לא נמצאו פריטים בקטלוג עבור', try:'נסו מק״ט, מותג, קטגוריה או משפחת דגם.', noFilter:'אין פריטים שתואמים לסינון שנבחר.', showMore:'הצגת מוצרים נוספים'});
  ...
  const imageMarkup=(p)=>p.imageUrl?`<img alt="${safeAttr(p.title||p.sku||'תמונת מוצר')}" loading="lazy" src="${safeAttr(p.imageUrl)}"/>`:`<span>${escapeHtml((p.brand||p.title||'S').trim().charAt(0)||'S')}</span>`;
  const productCard=(p)=>{
    const brand=p.brand||'Secure Smart'; const sku=p.sku||''; const title=p.title||sku||'פריט קטלוג'; const cat=p.category||'קטלוג'; const availability=p.availability||UI().toConfirm;
    ...
    const cartonOption=hasCartonOption?`<div class="ct-buy-option ct-carton-option"><div class="ct-buy-price-copy"><span class="ct-price-label">${ui.carton}</span>${vipBadge()}<b class="ct-price-amount" data-carton-price-display>${formatUsd(carton)}</b><span class="ct-vat-note">לפני מע״מ</span></div><div aria-label="כמות קרטונים מלאים" class="ct-qty-stepper"><button aria-label="הגדלת כמות קרטונים מלאים" data-qty-step="1" data-qty-target="carton" type="button">+</button><input aria-label="כמות קרטונים מלאים" data-carton-qty inputmode="numeric" min="0" type="number" value="0"/><button aria-label="הקטנת כמות קרטונים מלאים" data-qty-step="-1" data-qty-target="carton" type="button">−</button></div></div>`:'';
    return `<article class="ct-product-card ct-product-row" data-brand="${safeAttr(bslug)}" data-brand-label="${safeAttr(brand)}" data-category="${safeAttr(cslug)}" data-name="${safeAttr(normalizeText(productHaystack(p)))}" data-sku="${safeAttr(sku)}" data-stock="${/in stock/i.test(availability)?'in':'review'}">
      ...
      <div class="ct-buy-column" data-buy-widget data-carton-price="${carton}" data-unit-price="${unit}" data-units-per-carton="${cartonSize}" data-carton-available="${hasCartonOption?'1':'0'}"><div class="ct-buy-option ct-unit-option"><div class="ct-buy-price-copy"><span class="ct-price-label">${ui.unit}</span>${vipBadge()}<b class="ct-price-amount" data-unit-price-display>${formatUsd(unit)}</b><span class="ct-vat-note">לפני מע״מ</span></div><div aria-label="כמות יחידות" class="ct-qty-stepper"><button aria-label="הגדלת כמות יחידות" data-qty-step="1" data-qty-target="unit" type="button">+</button><input aria-label="כמות יחידות" data-unit-qty inputmode="numeric" min="0" type="number" value="1"/><button aria-label="הקטנת כמות יחידות" data-qty-step="-1" data-qty-target="unit" type="button">−</button></div></div>${cartonOption}<div class="ct-buy-total">${ui.total}: <strong data-price-total>${formatUsd(unit)}</strong></div><button data-add-cart data-brand="${safeAttr(brand)}" data-carton-price="${carton}" data-name="${safeAttr(title)}" data-sku="${safeAttr(sku)}" data-unit-price="${unit}" type="button">${ui.add}</button></div>
    </article>`;
  };
  ...
  const selectionLabel=(value)=>{if(!value||value==='all') return 'קטלוג מוצרים'; const parts=String(value).split(':'); if(parts[0]==='sourcepath'){const labels=(parts[2]||'').split('~').filter(Boolean).map(slug=>slug.replace(/-/g,' ')); return labels.length?labels[labels.length-1].replace(/\b\w/g,c=>c.toUpperCase()):categoryLabelBySlug[parts[1]]||'תת־קטגוריה';} return categoryLabelBySlug[value]||'קטגוריה';};
  ...
  const renderDiscompNav=(products)=>{
    ...
    if(!nav){nav=document.createElement('section'); nav.id='catalogDiscompNav'; nav.className='ct-discomp-nav ct-brand-discomp-nav'; nav.setAttribute('aria-label','ניווט קטלוג לפי מותג'); main.prepend(nav);}
    ...
    let title='קטלוג מוצרים';
    let tiles=[];
    if(currentNavSelection==='all'){
      title='בחירה לפי מותג';
      ...
      title=categoryLabelBySlug[selectedPrimary]||'קטגוריה';
      ...
      title=path[path.length-1]||categoryLabelBySlug[selectedPrimary]||'תת־קטגוריה';
    ...
    if(selectedPrimary) pushCrumb(categoryLabelBySlug[selectedPrimary]||'קטגוריה',`brandprimary:${selectedBrand}:${selectedPrimary}`);
    ...
    const backButton=parentNav?`<button type="button" class="ct-nav-back" data-category-nav="${safeAttr(parentNav)}" aria-label="חזרה לרמת קטלוג קודמת">‹ חזרה</button>`:`<span class="ct-nav-root" aria-label="רמה ראשית">מותגים</span>`;
    const tileMarkup=tiles.length?tiles.map(tile=>{const bnode=brandBySlug[tile.brandSlug]; const icon=tile.type==='brand'?logoForBrand(bnode):`<span aria-hidden="true">${escapeHtml(iconForLabel(tile.label))}</span>`; const action=tile.type==='brand'?'פתיחת קטגוריות מותג':(tile.type==='primary'?'פתיחת תתי־קטגוריות':'פתיחת תת־קטגוריה'); return `<button type="button" data-category-nav="${safeAttr(tile.value)}" class="ct-discomp-tile ${tile.value===currentNavSelection?'is-active':''}" aria-label="${safeAttr(`${action}: ${tile.label}, ${tile.count} מוצרים`)}">${icon}<b>${escapeHtml(tile.label)}</b><small>${tile.count.toLocaleString('he-IL')} מוצרים</small><em aria-hidden="true">›</em></button>`;}).join(''):`<div class="ct-discomp-empty">אין תת־קטגוריה נוספת לענף המותג הזה.</div>`;
    nav.innerHTML=`<div class="ct-brand-nav-panel"><div class="ct-nav-tools">${backButton}<nav class="ct-discomp-crumbs" aria-label="פירורי לחם">${crumbs.join('')}</nav></div><div class="ct-discomp-head"><h2>${escapeHtml(title)}</h2><span>${countForSelection(products,currentNavSelection).toLocaleString('he-IL')} מוצרים</span></div><div class="ct-discomp-tiles">${tileMarkup}</div></div>`;
  ...
  async function loadLiveCatalog(){
    ...
    try{
      ...
      updateSidebarFilters(liveProducts); renderCatalogExperience(liveProducts); renderDiscompNav(liveProducts); visibleProducts=liveProducts; { const ui=UI(); status.innerHTML=`<strong>${liveProducts.length.toLocaleString('he-IL')} ${ui.loaded}</strong> ${ui.loadedTail}`; } filterCatalog();
    }catch(err){status.innerHTML='<strong>נתוני הקטלוג לא נטענו.</strong> מוצגת רשימת גיבוי זמנית.'; filterCatalog();}
  }
  ...
  const selectionBreakdown=(sel)=>{const parts=[]; if(sel.cartonQty>0) parts.push(`${sel.cartonQty} קרטונים (${sel.cartonUnits} יחידות) במחיר קרטון`); if(sel.unitQty>0) parts.push(`${sel.unitQty} יחידות במחיר יחידה`); return parts.join(' + ');};
  ...
  const add=(btn,qty=1)=>{const name=btn.dataset.name||'פריט קטלוג'; const sku=btn.dataset.sku||'מק״ט'; const brand=btn.dataset.brand||''; const widget=buyWidgetFor(btn); const items=read(); let added=0; if(widget){refreshPriceWidget(widget); const sel=currentSelection(widget); addItem(items,{name,sku,brand,qty:sel.unitQty,mode:'unit',unitPrice:sel.unitPrice,lineTotal:sel.unitQty*sel.unitPrice,note:'מחיר יחידה'}); addItem(items,{name,sku,brand,qty:sel.cartonUnits,mode:'carton',unitPrice:sel.cartonPrice,lineTotal:sel.cartonUnits*sel.cartonPrice,note:'מחיר ליחידה בקרטון מלא',packSize:sel.unitsPerCarton,cartons:sel.cartonQty}); added=sel.unitQty+sel.cartonUnits;} else {const amount=normalizeQty(qty); const unitPrice=Number(btn.dataset.unitPrice||0); addItem(items,{name,sku,brand,qty:amount,mode:'unit',unitPrice,lineTotal:amount*unitPrice,note:unitPrice?'unit price':L().sample}); added=amount;} if(added<1){addItem(items,{name,sku,brand,qty:1,mode:'unit',unitPrice:Number(btn.dataset.unitPrice||0),lineTotal:Number(btn.dataset.unitPrice||0),note:'מחיר יחידה'}); added=1;} write(items); document.dispatchEvent(new CustomEvent('ss-cart-added',{detail:{name,qty:added}}));};
  ...
  const renderCart=()=>{const root=document.getElementById('cartList'); if(!root) return; if(root.contains(document.activeElement)) document.activeElement.blur(); const ll=L(); const items=ensurePriced(read(),true); if(!items.length){root.innerHTML=`<div class="ct-empty">${ll.empty}</div>`; return;} root.innerHTML=items.map((item,i)=>`<div class="ct-cart-row"><div><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.brand||'')} · ${escapeHtml(item.sku||'')} · ${escapeHtml(item.note||ll.sample)}${escapeHtml(priceNote(item))}${itemLineTotal(item)?` · סה״כ ${escapeHtml(formatUsd(itemLineTotal(item)))}`:''}</small></div><div class="ct-qty"><button type="button" data-dec="${i}">−</button><input aria-label="${ll.qty}" data-cart-qty="${i}" inputmode="numeric" min="1" type="number" value="${Number(item.qty)||1}"/><button type="button" data-inc="${i}">+</button></div><button class="ct-remove" type="button" data-remove="${i}">${ll.remove}</button></div>`).join('');};
  ...
  const productImageDetail=(p)=>p.imageUrl?`<img alt="${safeAttr(p.title||p.sku||'תמונת מוצר')}" loading="lazy" src="${safeAttr(p.imageUrl)}"/>`:`<div class="ct-image-fallback"><span>${escapeHtml((p.brand||p.title||'S').trim().charAt(0)||'S')}</span><small>אין תמונת מוצר זמינה</small></div>`;
  ...
  const renderProductDetail=(p)=>{
    ...
    if(info){ const h1=info.querySelector('h1'); if(h1) h1.textContent=title; const lead=info.querySelector('.ct-lead'); if(lead) lead.textContent=desc.length>260?desc.slice(0,257)+'…':desc; const meta=info.querySelector('.ct-meta-grid'); const warrantyClass=/\b24\s*months?\b/i.test(warranty)?' class="ct-warranty-highlight"':''; if(meta) meta.innerHTML=`<div><span>מותג</span><b>${escapeHtml(brand)}</b></div><div><span>מק״ט</span><b>${escapeHtml(sku)}</b></div><div><span>קטגוריה</span><b>${escapeHtml(cat)}</b></div><div><span>זמינות</span><b>${escapeHtml(availability)}</b></div><div${warrantyClass}><span>אחריות</span><b>${escapeHtml(warranty)}</b></div><div><span>Unit / carton</span><b>${escapeHtml(unitPerCarton)}</b></div>`; }
    ...
    const descBody=document.querySelector('.ct-description-layout > div'); if(descBody) descBody.innerHTML=`<p>${escapeHtml(desc)}</p><h3>זהות קטלוגית</h3><ul><li>מותג: ${escapeHtml(brand)}</li><li>מק״ט / דגם: ${escapeHtml(sku)}</li><li>קטגוריה: ${escapeHtml(cat)}</li><li>זמינות: ${escapeHtml(availability)}</li></ul><p class="ct-download-note">Secure Smart keeps the commercial order workflow separate from technical documents. Use the document section below for official English datasheets and manufacturer product lookups.</p>`;
    const params=document.querySelector('.ct-parameters-grid'); if(params) params.innerHTML=`<section><h3>נתוני קטלוג מסחריים</h3><dl><dt>מק״ט</dt><dd>${escapeHtml(sku||UI().toConfirm)}</dd><dt>מותג</dt><dd>${escapeHtml(brand)}</dd><dt>אחריות</dt><dd>${escapeHtml(warranty)}</dd><dt>יחידות בקרטון</dt><dd>${escapeHtml(unitPerCarton)}</dd></dl></section>${shippingDimensionSection(p)}<section><h3>סטטוס מידע טכני</h3><dl><dt>שפת מפרט</dt><dd>מקורות יצרן רשמיים</dd><dt>Document status</dt><dd>Official manufacturer data and verified public technical documents</dd><dt>Review status</dt><dd>${escapeHtml(p.enrichmentStatus||'Catalogue data under review')}</dd></dl></section>`;
  ...
  async function loadProductDetail(){
    ...
    try{
      ...
      try{
        ...
        if(lead) lead.textContent='המוצר לא נטען מנתוני הקטלוג. חזרו לקטלוג ופתחו אותו שוב.';
      }
    }
  }
