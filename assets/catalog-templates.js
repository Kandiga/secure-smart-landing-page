(function(){
  ...
  const fetchJsonCached=(url)=>{
    const versionedUrl=url.includes('?')?url+'&v=20260519-discomp-source-tree':url+'?v=20260519-discomp-source-tree';
  ...
  const isInStockProduct=(p)=>{const qty=Number(p.stockQtyVisible); const availability=normalizeText(p.availability||''); if(Number.isFinite(qty)&&qty>0) return true; if(/not\s+in\s+stock|expected|to\s+order|unavailable|out\s+of\s+stock/.test(availability)) return false; return /\bin\s+stock\b/.test(availability);};
  ...
  const productCard=(p)=>{
    ...
    return `<article class="ct-product-card ct-product-row" data-brand="${safeAttr(bslug)}" data-brand-label="${safeAttr(brand)}" data-category="${safeAttr(cslug)}" data-name="${safeAttr(normalizeText(productHaystack(p)))}" data-sku="${safeAttr(sku)}" data-stock="${isInStockProduct(p)?'in':'review'}">
