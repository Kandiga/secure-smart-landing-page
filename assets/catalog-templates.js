(function(){
  ...
  const productCard=(p)=>{
    ...
    const cartonOption=hasCartonOption?`<div class="ct-buy-option ct-carton-option"><div class="ct-buy-price-copy"><span class="ct-price-label">${ui.carton}</span><b class="ct-price-amount" data-carton-price-display>${formatUsd(carton)}</b><span class="ct-vat-note">exc. VAT</span></div><div aria-label="Full carton quantity" class="ct-qty-stepper"><button aria-label="Increase full carton quantity" data-qty-step="1" data-qty-target="carton" type="button">+</button><input aria-label="Full carton quantity" data-carton-qty inputmode="numeric" min="0" type="number" value="0"/><button aria-label="Decrease full carton quantity" data-qty-step="-1" data-qty-target="carton" type="button">−</button></div></div>`:'';
    return `<article class="ct-product-card ct-product-row" data-brand="${safeAttr(bslug)}" data-brand-label="${safeAttr(brand)}" data-category="${safeAttr(cslug)}" data-name="${safeAttr(normalizeText(productHaystack(p)))}" data-sku="${safeAttr(sku)}" data-stock="${/in stock/i.test(availability)?'in':'review'}">
      ...
      <div class="ct-product-copy"><h2><a href="${href}">${escapeHtml(title)}</a></h2><dl><dt>${ui.sku}</dt><dd>${escapeHtml(sku||ui.toConfirm)}</dd><dt>${ui.brand}</dt><dd>${escapeHtml(brand)}</dd><dt>${ui.category}</dt><dd>${escapeHtml(cat)}</dd><dt>${ui.warranty}</dt><dd>${escapeHtml(warranty)}</dd><dt>${ui.availability}</dt><dd>${escapeHtml(availability)}</dd></dl></div>
      <div class="ct-buy-column" data-buy-widget data-carton-price="${carton}" data-unit-price="${unit}" data-units-per-carton="${cartonSize}" data-carton-available="${hasCartonOption?'1':'0'}"><div class="ct-buy-option ct-unit-option"><div class="ct-buy-price-copy"><span class="ct-price-label">${ui.unit}</span><b class="ct-price-amount" data-unit-price-display>${formatUsd(unit)}</b><span class="ct-vat-note">exc. VAT</span></div><div aria-label="Unit quantity" class="ct-qty-stepper"><button aria-label="Increase unit quantity" data-qty-step="1" data-qty-target="unit" type="button">+</button><input aria-label="Unit quantity" data-unit-qty inputmode="numeric" min="0" type="number" value="1"/><button aria-label="Decrease unit quantity" data-qty-step="-1" data-qty-target="unit" type="button">−</button></div></div>${cartonOption}<div class="ct-buy-total">${ui.total}: <strong data-price-total>${formatUsd(unit)}</strong></div><small class="ct-price-breakdown" data-price-breakdown></small><button data-add-cart data-brand="${safeAttr(brand)}" data-carton-price="${carton}" data-name="${safeAttr(title)}" data-sku="${safeAttr(sku)}" data-unit-price="${unit}" type="button">${ui.add}</button></div>
    </article>`;
  };
